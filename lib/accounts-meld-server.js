...
AM.prototype._config = {
    ...
    meldDBCallback: null,
    // Reference to the callback used to update user profile when a service is added
...
AM.prototype._meldUsersObject = function(src_user, dst_user) {
    ...
    _.each(_.keys(src_services), function(service_name){
        ...
        dst_user.services[service_name] = src_services[service_name];
    });
    ...
    AccountsEmailsField.updateEmails({user: dst_user});
    ...
    if (currentUser) {
        ...
        if (!currentUser.services[serviceName]) {
            ...
            if (!user) {
                ...
                // Updates the registered_emails field
                AccountsEmailsField.updateEmails({user: currentUser});
                // Checks whether a callback for user update after a new service is added was specified
                var serviceAddedCallback = AccountsMeld.getConfig('serviceAddedCallback');
                if (serviceAddedCallback)
                    serviceAddedCallback(currentUser._id, serviceName);
        ...
        if (!AccountsMeld.getConfig('askBeforeMeld')){
            ...
            if (!user) {
                ...
                var serviceEmail = AccountsEmailsField.getEmailsFromService(serviceName, serviceData);
